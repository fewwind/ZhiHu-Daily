...
public class UniversalAndroidImageLoader implements ImageLoaderWrapper {
    ...
    public void displayImage(ImageView imageView, File imageFile, DisplayOption option) {
        int imageLoadingResId = R.mipmap.default_icon;
        int imageErrorResId = R.mipmap.default_icon;
        if (option != null) {
    ...
    @Override
    public void displayImage(ImageView imageView, String imageUrl, DisplayImageOptions option) {
        int imageLoadingResId = R.mipmap.default_icon;
        int imageErrorResId = R.mipmap.default_icon;
...
//            String uri = ImageDownloader.Scheme.HTTP.wrap(imageUrl);
            ...
            if (option == null)
                ImageLoader.getInstance().displayImage(imageUrl, imageView, options);
            else ImageLoader.getInstance().displayImage(imageUrl, imageView, option);
        }
    }
